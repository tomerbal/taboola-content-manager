...
$(document).ready(function(){getCountries();getBrands();getAllData(0);initFilter();initInsert()});
...
$("#genericCode").text(a.status);$("#genericModal").modal()})}function getAllData(a){$.get("/content-item/get-all-content-items/"+a+"/"+config.batchSize,function(b){200!==b.status?($("#genericMessage").text("There was an error retrieving the data"),$("#genericCode").text(b.status),$("#genericModal").modal()):(fillDashboard(a,b),initDashboard())}).fail(function(a){$("#genericMessage").text("There was an error retrieving the data");$("#genericCode").text(a.status);$("#errorModal").modal()})}
function initDashboard(){$('[data-toggle="tooltip"]').tooltip();$('table tbody input[type="checkbox"]').click(function(){this.checked||$("#selectAll").prop("checked",!1)})}
...
d.img);e.attr("itemId",d.id);$("#dashboardTableBody").append(e)}})}function paging(a){$(".pagination").find(".active").removeClass("active");a=parseInt(a.innerText)-1;0<Object.keys(config.filters).length?getFilteredData(a,!0):getAllData(a)};
