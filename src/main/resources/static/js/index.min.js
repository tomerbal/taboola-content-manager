...
e+"</a></li>"):d.after('<li class="page-item"><a class="page-link" onclick="paging(this)">'+e+"</a></li>"),d=$(".page-item").last();$.get("../htmls/item-content.html",function(a){for(var c in b.contentItems){var d=b.contentItems[c],e=$($.parseHTML(a)),f=e.find(".itemContentInputCheckbox");f.attr("id","checkbox"+c);config.chosenItems[d.id]&&f.prop("checked",!0);e.find(".itemContentInputLabel").attr("for","checkbox"+c);e.find(".country").text(d.countryName);e.find(".brand").text(d.brandName);e.find(".title").text(d.title);
e.find(".img img").attr("src",d.img);e.attr("itemId",d.id);$("#dashboardTableBody").append(e)}initDashboard()})}function paging(a){$(".pagination").find(".active").removeClass("active");a=parseInt(a.innerText)-1;0<Object.keys(config.filters).length?getFilteredData(a,!0):getAllData(a)};
